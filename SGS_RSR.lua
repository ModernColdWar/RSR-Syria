...
--  AllGroups = SET_GROUP:New():FilterCategories("ground"):FilterActive(true):FilterStart()
  ...
  dofile("SaveUnits_RSR.lua")
  tempTable={}
  Spawn={}
--RUN THROUGH THE KEYS IN THE TABLE (GROUPS)
  for k,v in pairs (SaveUnits_RSR) do
    units={}
--RUN THROUGH THE UNITS IN EACH GROUP
      for i= 1, #(SaveUnits_RSR[k]["units"]) do 
  
tempTable =

  { 
   ["type"]=SaveUnits_RSR[k]["units"][i]["type"],
   ["transportable"]= {["randomTransportable"] = false,}, 
   --["unitId"]=9000,used to generate ID's here but no longer doing that since DCS seems to handle it
   ["skill"]=SaveUnits_RSR[k]["units"][i]["skill"],
   ["y"]=SaveUnits_RSR[k]["units"][i]["y"] ,
   ["x"]=SaveUnits_RSR[k]["units"][i]["x"] ,
   ["name"]=SaveUnits_RSR[k]["units"][i]["name"],
...
groupData = 

  {
    ...
    ["y"] = SaveUnits_RSR[k]["y"],
    ["x"] = SaveUnits_RSR[k]["x"],
    ["name"] = SaveUnits_RSR[k]["name"],
    --["start_time"] = 0,
  } 

  coalition.addGroup(SaveUnits_RSR[k]["CountryID"], SaveUnits_RSR[k]["CategoryID"], groupData)
  groupData = {}
